...
class Program{
    public:
        int program_number,program_arrival_time,program_burst_time,program_completion_time,program_turnaround_time,program_waiting_time,program_burst_time_demo;
        void getdata(int a,int b,int c)
        {
            program_number = a;
            program_arrival_time = b;
            program_burst_time = c;
            program_burst_time_demo = c;
        }
};
...
int main()
{
    ...
    while(completed_process < size)
    {
        ...
        for(int i = 0;i < size;i++)
        {
            ...
            {
                if(p[i].program_burst_time_demo < min_burst_time || (p[i].program_burst_time_demo == min_burst_time && p[i].program_arrival_time < p[idx].program_arrival_time))
                {
                    min_burst_time = p[i].program_burst_time_demo;
                    idx = i;
                }
            } 
        }
        ...
        if(idx != -1)
        {
            p[idx].program_completion_time = current_time + 1;
            p[idx].program_burst_time_demo--;

            current_time = p[idx].program_completion_time;
            if(p[idx].program_burst_time_demo == 0)
            {
